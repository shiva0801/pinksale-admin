...
import {
  addLounchPadData,
} from "../../../App/redux/features/FormStore/FormSlice";
...
const AddAdditionalInfo = () => {
  ...
  const validate = () => {
    form.validateFields().then((values) => {
      dispatch(addLounchPadData({ ...values, lounchpadForm : 3 , bar:3}));
    });
  };

  const initialValues = {
    logo: lounchPadTab?.lounchPadData?.logo ||"",
    website: lounchPadTab?.lounchPadData?.website ||"",
    facebook: lounchPadTab?.lounchPadData?.facebook ||"",
    twitter: lounchPadTab?.lounchPadData?.twitter ||"",
    Github: lounchPadTab?.lounchPadData?.Github ||"",
    telegram: lounchPadTab?.lounchPadData?.telegram ||"",
    instagram: lounchPadTab?.lounchPadData?.instagram ||"",
    discord: lounchPadTab?.lounchPadData?.discord ||"",
    Reddit: lounchPadTab?.lounchPadData?.Reddit ||"",
    Youtube: lounchPadTab?.lounchPadData?.Youtube ||"",
    description: lounchPadTab?.lounchPadData?.description ||"",
  };
      ...
      <Form form={form} onFinish={validate} initialValues={initialValues}>
        ...
        <div className="bnb_description">
          ...
          <div className="bnb_description_right_main">
            ...
            <Form.Item
              name="website"
              rules={[{ required: true, message: "website cannot be blank" }]}
            >
              <Input type={"text"} />
            </Form.Item>
      ...
      <div className="back_and_next_button_section">
        <div className="back_and_next_button">
          <button
            ...
            onClick={() => {
              dispatch(addLounchPadData({lounchpadForm : 1 , bar:1}));
            }}
          >
            Back
          </button>
