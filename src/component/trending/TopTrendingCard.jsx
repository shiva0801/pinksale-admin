...
import { CustomButton, Heading, Text, TredingCard } from './toptrendingcard.styled'
    ...
    return (
        <>
            ...
            <Grid container spacing={5} sx={{ paddingInline: 10 }}>
                {trendingdata && trendingdata.map((cardData, index) => (
                    <Grid item xs={12} md={6} lg={3} key={index} p={0}>
                        <TredingCard sx={{
                            ...
                            <CardActions>
                                <CustomButton onClick={handlerTrendingModal} size='small' variant='outlined'>Edit</CustomButton>
                            </CardActions>
                        </TredingCard>
                    </Grid>
                ))}
